...
class CubeAPI:
    ...
    def get_backorders(self):
        ...
        try:
            ...
            logger.error(f"Failed to fetch inkooporders: {str(e)}")
            raise

    def store_backorders(self, backorders_data):
        """Store inkooporders data in SQLite database"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        # Changed from inkoopOrderLines to backOrderLines to match the API response
        logger.info(f"Processing backorders data with {len(backorders_data.get('backOrderLines', []))} lines")
        for line in backorders_data.get('backOrderLines', []):
            try:
                cursor.execute('''
                    INSERT OR REPLACE INTO inkooporders 
                ...
                logger.info(f"Successfully stored order line {line['cubeLineId']}")
            except Exception as e:
                logger.error(f"Error storing order line {line.get('cubeLineId')}: {str(e)}")
                continue

        conn.commit()
        conn.close()
        logger.info("Successfully completed storing all backorders in database")

    def sync_backorders(self):
        """Fetch and store inkooporders data"""
        try:
